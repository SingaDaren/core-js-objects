...
function sellTickets(queue) {
  return (
    queue.slice(0, -1).reduce((acc, elem) => acc + elem, 0) >=
    queue.slice(-1) - 25
  );
}
...
const cssSelectorBuilder = {
  ...
  selectorCheck(value, order, selector = null) {
    ...
    const newObj = { ...this };
    newObj.result += value;
    if (selector) newObj[selector] = true;
    newObj.lastOrder = order;
    return newObj;
  },

  element(value) {
    return this.selectorCheck(value, 1, 'hasElement');
  },

  id(value) {
    return this.selectorCheck(`#${value}`, 2, 'hasId');
  },

  class(value) {
    return this.selectorCheck(`.${value}`, 3);
  },

  attr(value) {
    return this.selectorCheck(`[${value}]`, 4);
  },

  pseudoClass(value) {
    return this.selectorCheck(`:${value}`, 5);
  },

  pseudoElement(value) {
    return this.selectorCheck(`::${value}`, 6, 'hasPseudo');
  },
